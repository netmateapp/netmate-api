...
use crate::{common::{handle::{id::HandleId, share_count::HandleShareCount}, id::account_id::AccountId}, helper::{error::InitError, middleware::{rate_limiter, session_manager, TimeUnit}, redis::Pool}};
...
pub async fn endpoint(db: Arc<Session>, cache: Arc<Pool>) -> Result<Router, InitError<CountHandlesShareImpl>> {
    ...
    let get_handles = CountHandlesShareImpl::try_new(db).await?;
