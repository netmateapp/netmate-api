use crate::{common::{fallible::Fallible, tag::{non_top_tag::NonTopTagId, redis_tag_info::{RedisTagInfo, TagListOrder}, tag_name::TagName}}, endpoints::tag::proposal::propose::dsl::add_relation::{HierarchicalTagRelator, HierarchicalTagRelatorError}, helper::redis::{connection::conn, namespace::NAMESPACE_SEPARATOR, namespaces::{EQUIVALENT, SUB, SUPER, TAG_LIST}}};

use super::ProposeTagRelationImpl;

impl HierarchicalTagRelator for ProposeTagRelationImpl {
    async fn relate_by_inclusion(&self, subtag_id: NonTopTagId, subtag_name: TagName, supertag_id: NonTopTagId, supertag_name: TagName) -> Fallible<(), HierarchicalTagRelatorError> {
        let mut conn = conn(&self.cache, |e| HierarchicalTagRelatorError::RelateByInclusionFailed(e.into())).await?;
        ...
        self.insert_unstable_proposals_to_list
            ...
            .await
            .map_err(|e| HierarchicalTagRelatorError::RelateByInclusionFailed(e.into()))?;
