...
impl UnrateTagRelationImpl {
    pub async fn try_new(db: Arc<Session>) -> Fallible<Self, InitError<Self>> {
        ...
        let delete_tag_relation_rating_from_account = prepare(&db, " DELETE FROM tag_relation_ratings_by_account WHERE account_id = ? AND subtag_id = ? AND supertag_id = ? AND relation = ?").await?;

        let insert_tag_relation_rating_removal_into_cycle = prepare(&db, "INSERT INTO tag_relation_ratings (language_group, cycle, account_id, subtag_id, supertag_id, relation, operation_id) VALUES (?, ?, ?, ?, ?, ?, 127)").await?;
