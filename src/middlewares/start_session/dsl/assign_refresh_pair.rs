...
pub(crate)  trait AssignRefreshPair {
    async fn assign_refresh_pair(&self, session_account_id: AccountId, expiration: RefreshPairExpiration) -> Fallible<(SessionSeries, RefreshToken), AssignRefreshPairError> {
    ...
    async fn try_assign_refresh_pair_with_expiration_if_unused(&self, session_series: &SessionSeries, refresh_token: &RefreshToken, session_account_id: AccountId, expiration: RefreshPairExpiration) -> Fallible<(), AssignRefreshPairError>;
}
