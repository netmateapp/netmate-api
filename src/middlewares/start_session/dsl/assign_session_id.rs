...
pub(crate)  trait AssignSessionId {
    async fn assign_session_id(&self, session_account_id: &AccountId, new_expiration: &SessionExpirationSeconds) -> Fallible<SessionId, AssignSessionIdError> {
        ...
        loop {
            match self.try_assign_new_session_id_with_expiration_if_unused(&new_session_id, session_account_id, new_expiration).await {
                ...
                Err(AssignSessionIdError::SessionIdAlreadyUsed) => new_session_id = SessionId::gen(),
                Err(e) => return Err(e),
            }
        }
    }

    async fn try_assign_new_session_id_with_expiration_if_unused(&self, new_session_id: &SessionId, session_account_id: &AccountId, new_expiration: &SessionExpirationSeconds) -> Fallible<(), AssignSessionIdError>;
}
