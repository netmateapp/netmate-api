use crate::{common::{fallible::Fallible, id::account_id::AccountId, session::{session_expiration::SessionExpirationSeconds, session_id::SessionId}}, helper::redis::TypedCommand, middlewares::{manage_session::dsl::update_session::{UpdateSession, UpdateSessionError}, session::{SessionIdOld, SetSessionIdCommand}}};
...
impl UpdateSession for ManageSessionImpl {
    async fn try_assign_new_session_id_with_expiration_if_unused(&self, new_session_id: &SessionId, session_account_id: AccountId, new_expiration: SessionExpirationSeconds) -> Fallible<(), UpdateSessionError> {
        let key = SessionIdOld(new_session_id);
